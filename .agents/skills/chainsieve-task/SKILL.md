...
1. Run every verification command in the immutable goal and task contract, including task-owned tests and control-plane-owned conformance tests.
...
4. Run the authoritative `task:self-review` command with the bound holder and fencing version.
5. Run the real provider-independent `task:verify` command with the same bindings.
6. Stop. Never invoke the merge queue and never start another task.
