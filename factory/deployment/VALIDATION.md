...
- PASS — 56 deterministic Python tests after the pre-VPS hardening pass both locally and in an unprivileged Linux/amd64 Python 3.12 container with Git. They cover renewable token expiry/auth-failure rotation, including non-raising branch probes; installation-compatible post-action actor verification; immutable/elevated path authority; real Muse→Agy, Agy→Muse, and stuck-worker alternate respawn; bounded Terra/Sol routing; global work keys; heartbeat liveness; read-only doctor behavior; actor overrides; conversation policy; root-checkout unit invariants; and the prior reconciliation/security suite.
