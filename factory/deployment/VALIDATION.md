...
Use only `factory/canary-base`. Never target `main`, merge PR #73, delete the legacy factory, or claim production cutover during validation. `PASS` requires durable evidence from the actual target; deterministic mocks leave live rows `NOT_RUN`.
