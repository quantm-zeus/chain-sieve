...
const branchFromGeneratedGoal = async (
  ...
  const candidates = [
    `${contract.id}.agent-goal.md`,
    `${contract.id}.zcode-goal.md`,
  ];
  for (const file of candidates) {
    const path = join(root, 'clusters', contract.group, file);
...
const verifyContextManifest = async (
  ...
  for (const file of manifest.files) {
    if (contextPaths.has(file.path))
      throw new ZCodeError('DUPLICATE_CONTEXT_REFERENCE', file.path);
  ...
  if (manifest.conformanceManifestPath && manifest.conformanceManifestSha256) {
    const conformance = await readFile(
      join(root, manifest.conformanceManifestPath),
    );
    if (sha256(conformance) !== manifest.conformanceManifestSha256)
      throw new ZCodeError('CONFORMANCE_MANIFEST_DRIFT', task.id);
  }
  return {
    path: relative(root, manifestPath),
    sha256: sha256(text),
    ...(manifest.conformanceManifestPath
      ? { conformanceManifestPath: manifest.conformanceManifestPath }
      : {}),
    ...(manifest.conformanceManifestSha256
      ? { conformanceManifestSha256: manifest.conformanceManifestSha256 }
      : {}),
  };
};
...
const assertGraphInventory = (
  ...
  for (const cluster of clusters) {
    ...
    const acceptance = [
      ...new Set([
        ...ownedTasks.flatMap((task) => task.acceptanceCriteria),
        ...cluster.integrationAcceptanceCriteria,
      ]),
    ].sort();
    if (
...
const coverage = async (
  ...
  const levelPath = join(root, 'tasks/generated/acceptance-level-map.json');
  const acceptanceLevels = existsSync(levelPath)
    ? await readJson<
        Record<
          string,
          { level: 'TASK' | 'CLUSTER' | 'PROJECT'; owner: string }
        >
      >(levelPath)
    : Object.fromEntries(
        Object.entries(
          await readJson<Record<string, string[]>>(
            join(root, 'tasks/generated/acceptance-task-map.json'),
          ),
        ).map(([id, owners]) => [
          id,
  ...
  const badAcceptance = spec.acceptanceCriteria
    .filter((item) => !acceptanceLevels[item.id])
    .map((item) => item.id);
  const invalidLevelOwners = Object.entries(acceptanceLevels)
    .filter(([, value]) =>
      value.level === 'TASK'
        ? !tasks.some((task) => task.id === value.owner)
        : value.level === 'CLUSTER'
          ? !/^C-G[0-7]-/.test(value.owner)
          : value.owner !== 'PROJECT',
    )
    .map(([id]) => id);
  ...
  const unknownAcceptance = Object.keys(acceptanceLevels).filter(
  ...
  return {
    ...
    acceptanceCriteria: {
      accounted: Object.keys(acceptanceLevels).length,
...
export const discoverProject = async (
  ...
  for (const input of clusterInputs) {
    ...
    clusterRecords.push({
      ...
      goalPath: `clusters/${input.contract.group}/${input.contract.id}.agent-goal.md`,
      branch,
      state: clusterState(
        input.contract,
        lifecycle.tasks,
  ...
  for (const task of tasks) {
    ...
    if (persistedBinding) {
      ...
      const legacyContractPath = join(
        workspace,
        'tasks',
        task.dependencyGroup,
        `${task.id}.contract.json`,
      );
      if (existsSync(legacyContractPath)) {
        const legacyText = await readFile(legacyContractPath, 'utf8');
    ...
    const context = await verifyContextManifest(contextRoot, effectiveTask);
