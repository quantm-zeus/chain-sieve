...
import { CLUSTER_POLICY_VERSION, CLUSTER_VERIFIER_VERSION, deriveClusterTaskAttestations, validateClusterResult, validateClusterReview } from './verification.js';
...
try {
  ...
  if (command === 'list') console.log(JSON.stringify(clusters.map((cluster) => ({ id: cluster.id, tasks: cluster.tasks.length })), null, 2));
    ...
    if (command === 'verify') {
      const head = git(['rev-parse', 'HEAD']); const tree = git(['rev-parse', 'HEAD^{tree}']); if (git(['status', '--porcelain']) !== '') throw new Error('DIRTY_WORKTREE'); const taskAttestations = await deriveClusterTaskAttestations(cluster, tasks, state, head);
      ...
      const specification = await loadAndValidateSpecification(); const integrationTests = [...new Set(specification.manifest.acceptanceCriteria.filter((item) => cluster.integrationAcceptanceCriteria.includes(item.id)).flatMap((item) => [item.positiveTestRef, item.negativeOrFailureTestRef]))]; const evidence = [...(integrationTests.length > 0 ? [run(['exec', 'vitest', 'run', ...integrationTests])] : []), run(['test:integration']), run(['test:system']), run(['architecture:verify']), run(['placeholders:scan']), run(['prohibited-capabilities:scan']), run(['migration:verify'])]; const commandEvidence = await persistCommandEvidence(cluster.id, head, evidence); const contractText = await readFile(clusterContractPath(cluster.group, cluster.id), 'utf8'); const result = ClusterResultSchema.parse({ schemaVersion: '2.0.0', clusterId: cluster.id, status: 'PASS', clusterContractSha256: sha256(contractText), headCommitSha: head, headTreeSha: tree, taskAttestations, commandEvidence, verifierVersion: CLUSTER_VERIFIER_VERSION, verificationPolicyVersion: CLUSTER_POLICY_VERSION, verificationTimestamp: new Date().toISOString() }); await validateClusterResult(cluster, result, tasks, state); const path = resultPath(cluster.id); await mkdir(dirname(path), { recursive: true }); await writeFile(path, `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 }); console.log(JSON.stringify({ clusterId: cluster.id, status: 'VERIFIED', taskResults: taskAttestations.length, integrationAcceptanceCriteria: cluster.integrationAcceptanceCriteria, resultSha256: sha256(JSON.stringify(result)) }, null, 2));
    } else if (command === 'report') {
      const rawResult = ClusterResultSchema.parse(JSON.parse(await readFile(resultPath(cluster.id), 'utf8'))); const result = await validateClusterResult(cluster, rawResult, tasks, state, process.cwd(), rawResult.headCommitSha); const rawReview = ClusterReviewSchema.parse(JSON.parse(await readFile(join('artifacts/reviews/clusters', `${cluster.id}.review.json`), 'utf8'))); const implementationIdentities = cluster.tasks.map((taskId) => state.tasks[taskId]?.holder).filter((holder): holder is string => Boolean(holder)); const review = await validateClusterReview(cluster, result, rawReview, process.cwd(), implementationIdentities); console.log(JSON.stringify({ clusterId: cluster.id, status: 'REVIEWED', result, review }, null, 2));
