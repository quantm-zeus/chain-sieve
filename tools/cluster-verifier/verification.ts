...
const git = (args: string[], cwd = process.cwd()): string => {
  ...
  if (result.status !== 0)
    throw new Error(
      `CLUSTER_GIT_FAILED:${args.join(':')}:${result.stderr.trim()}`,
    );
...
> => {
  ...
  for (const taskId of cluster.tasks) {
    ...
    if (taskState?.state !== 'MERGED' || !taskState.commit)
      throw new Error(
        `CLUSTER_TASK_NOT_MERGED:${taskId}:${taskState?.state ?? 'MISSING'}`,
      );
    const generatedTask = tasks.find((item) => item.id === taskId);
    if (!generatedTask)
      throw new Error(`CLUSTER_TASK_CONTRACT_MISSING:${taskId}`);
    ...
    const task =
      ...
      lifecycle.conformanceManifestSha256
        ? {
            ...
            testQualityGate:
              boundTask.riskLevel === 'HIGH' ||
              boundTask.riskLevel === 'CRITICAL'
    ...
    const result = await validateTaskAttestation(task, JSON.parse(text), {
      cwd,
      clusterHead,
      state: taskState,
    });
  ...
  if (attestations.length === 0)
    throw new Error('EMPTY_CLUSTER_TASK_EVIDENCE');
  return attestations;
};
...
export const validateClusterResult = async (
  ...
  try {
    ...
    throw new Error(
      `CLUSTER_RESULT_SCHEMA_INVALID:${error instanceof Error ? error.message : String(error)}`,
    );
  }
  if (result.status !== 'PASS' || result.clusterId !== cluster.id)
    throw new Error('FORGED_CLUSTER_PASS');
  const head = expectedProductCommit ?? git(['rev-parse', 'HEAD'], cwd);
  const tree = git(['rev-parse', `${head}^{tree}`], cwd);
  if (result.headCommitSha !== head)
    throw new Error('CLUSTER_HEAD_COMMIT_MISMATCH');
  ...
  const contractText = await readFile(
    join(cwd, clusterContractPath(cluster)),
    'utf8',
  );
  if (result.clusterContractSha256 !== sha256(contractText))
    throw new Error('WRONG_CLUSTER_CONTRACT_HASH');
  ...
  if (result.commandEvidence.length === 0)
    throw new Error('EMPTY_CLUSTER_COMMAND_EVIDENCE');
  for (const item of result.commandEvidence)
...
interface ReviewSessionReceipt {
  schemaVersion: '2.0.0';
  sessionId: string;
  reviewerIdentity: string;
  provider: 'antigravity' | 'codex' | 'zcode';
  clusterId: string;
  productCommit: string;
  productTree: string;
  implementationHolders: string[];
  createdAt: string;
  receiptHash: string;
}

const validateReviewSessionReceipt = async (
  cluster: ClusterContract,
  reviewerIdentity: string,
  productCommit: string,
  productTree: string,
  ...
  const directory = join(runtimeRoot(cwd), 'agent', 'review-sessions');
  let names: string[];
  try {
    names = await readdir(directory);
  } catch {
    throw new Error('CLUSTER_REVIEW_SESSION_RECEIPT_MISSING');
  }
  for (const name of names.filter((item) => item.endsWith('.json'))) {
    const raw = await readFile(join(directory, name), 'utf8');
    let receipt: ReviewSessionReceipt;
    try {
      receipt = JSON.parse(raw) as ReviewSessionReceipt;
    } catch {
      continue;
    }
    if (receipt.reviewerIdentity !== reviewerIdentity) continue;
    const { receiptHash, ...core } = receipt;
    if (
      receipt.schemaVersion !== '2.0.0' ||
      !['antigravity', 'codex', 'zcode'].includes(receipt.provider) ||
      sha256(JSON.stringify(core)) !== receiptHash ||
      receipt.clusterId !== cluster.id ||
      receipt.productCommit !== productCommit ||
      receipt.productTree !== productTree ||
      Date.parse(receipt.createdAt) > Date.now() ||
      implementationIdentities.includes(receipt.reviewerIdentity) ||
      JSON.stringify([...receipt.implementationHolders].sort()) !==
        JSON.stringify([...implementationIdentities].sort())
    )
      throw new Error('CLUSTER_REVIEW_SESSION_RECEIPT_INVALID');
    return;
  }
  throw new Error('CLUSTER_REVIEW_SESSION_RECEIPT_MISSING');
};
...
export const validateClusterReview = async (
  ...
  try {
    ...
    throw new Error(
      `CLUSTER_REVIEW_SCHEMA_INVALID:${error instanceof Error ? error.message : String(error)}`,
    );
  }
  const resultText = await readFile(
    join(runtimeRoot(cwd), 'cluster-results', `${cluster.id}.result.json`),
    'utf8',
  );
  if (review.clusterId !== cluster.id)
    throw new Error('CLUSTER_REVIEW_WRONG_CLUSTER');
  if (implementationIdentities.includes(review.reviewerIdentity))
    throw new Error('CLUSTER_REVIEWER_NOT_INDEPENDENT');
  if (review.reviewedProductCommit !== result.headCommitSha)
    throw new Error('CLUSTER_REVIEW_WRONG_PRODUCT_COMMIT');
  if (review.reviewedProductTree !== result.headTreeSha)
    throw new Error('CLUSTER_REVIEW_WRONG_PRODUCT_TREE');
  await validateReviewSessionReceipt(
    cluster,
  ...
  if (review.clusterContractSha256 !== result.clusterContractSha256)
    throw new Error('CLUSTER_REVIEW_WRONG_CONTRACT_HASH');
  if (
    review.clusterResultPath !== `cluster-results/${cluster.id}.result.json` ||
    review.clusterResultSha256 !== sha256(resultText)
  )
    throw new Error('CLUSTER_REVIEW_WRONG_RESULT_HASH');
  const expectedAttestations = result.taskAttestations.map((item) => ({
    taskId: item.taskId,
    resultSha256: item.resultSha256,
  }));
  if (
    JSON.stringify(review.taskAttestationHashes) !==
    JSON.stringify(expectedAttestations)
  )
    throw new Error('CLUSTER_REVIEW_TASK_ATTESTATION_MISMATCH');
  if (Date.parse(review.reviewedAt) < Date.parse(result.verificationTimestamp))
    throw new Error('CLUSTER_REVIEW_PREDATES_RESULT');
  if (
    review.verdict !== 'PASS' ||
  ...
  if (
    review.reviewArtifactCommit &&
    review.reviewArtifactCommit !== reviewCommit
  )
  ...
  const changed = git(
    ['diff', '--name-only', `${result.headCommitSha}..${reviewCommit}`],
    cwd,
  )
    .split('\n')
    .filter(Boolean);
  if (JSON.stringify(changed) !== JSON.stringify([reviewPath]))
    throw new Error('CLUSTER_REVIEW_COMMIT_NOT_ARTIFACT_ONLY');
  return review;
};
