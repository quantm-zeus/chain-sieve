...
import {
  verifyPostMergeIntegration,
  type IntegrationRunner,
} from '../cluster-verifier/integration.js';
import {
  invalidateRebaseEvidence,
  registerCurrentEvidence,
} from '../task-runner/evidence-ledger.js';
...
export const retryRevertedIntegration = async (
  task: TaskContract,
  tasks: TaskContract[],
  cwd = process.cwd(),
): Promise<{
  ...
  status: 'MERGED';
  commit: string;
  integration: ReturnType<typeof verifyPostMergeIntegration>;
}> => {
  const state = await readState(tasks, cwd);
  ...
  if (
    ...
    throw new Error('REVERTED_INTEGRATION_TASK_REQUIRED');
  const directory = join(runtimeRoot(cwd), 'integration-failures', task.id);
  const files = (await readdir(directory))
    .filter((name) => name.endsWith('.json'))
    .sort();
  if (files.length === 0) throw new Error('INTEGRATION_FAILURE_AUDIT_REQUIRED');
  const audit = JSON.parse(
    await readFile(join(directory, files.at(-1)!), 'utf8'),
  ) as {
    taskId: string;
    taskCommit: string;
    revertCommit: string;
  ...
  if (integration.status !== 'PASS') {
    ...
    throw new Error(`INTEGRATION_RETRY_FAILED:${integration.outputSha256}`);
  }
  target.history ??= [];
  target.history.push({
    from: target.state,
    ...
    command: 'merge-queue:retry-integration',
...
const git = (args: string[], cwd: string, allowFailure = false): string => {
  ...
  if (result.status !== 0 && !allowFailure)
    throw new Error(
      `MERGE_QUEUE_GIT_FAILED:${args.join(':')}:${result.stderr.trim()}`,
    );
...
const taskWorktree = (taskId: string, cwd: string): string => {
  ...
  const block = blocks.find((value) =>
    value.includes(`branch refs/heads/${taskBranch(taskId)}`),
  );
  const path = block
    ?.split('\n')
...
const persistPostRebaseReceipt = async (
  ...
  const record = {
    ...
    contextPath: lifecycle.contextManifestPath.replace(
  ...
  const stored = await persistGoalAndPayload(
    ...
    runner,
    {
      ...
      failures: [
        'Post-rebase evidence was invalidated; perform a fresh semantic self-review.',
      ],
    },
    'antigravity',
  );
  if (!stored.binding.launchReceiptId)
    throw new Error('POST_REBASE_RECEIPT_ID_MISSING');
...
const assertPathLocks = (
  ...
  for (const other of Object.values(state.tasks)) {
    ...
    const otherContract = tasks.find(
      (candidate) => candidate.id === other.taskId,
    );
    if (
      otherContract?.exclusiveLocks.some((lock) =>
        task.exclusiveLocks.includes(lock),
      )
    )
...
export const enqueueTask = async (
  ...
  if (git(['rev-parse', 'HEAD'], worktree) !== target.commit)
    throw new Error('TASK_WORKTREE_HEAD_MISMATCH');
  ...
  if (
    queue.items.some(
      (candidate) =>
        candidate.taskId === task.id && candidate.status === 'QUEUED',
    )
  )
...
export const processMergeQueue = async (
  ...
  if (currentBranch !== clusterBranch)
    throw new Error(`CLUSTER_BRANCH_REQUIRED:${clusterBranch}`);
  if (git(['status', '--porcelain'], cwd) !== '')
    throw new Error('CLUSTER_WORKTREE_NOT_CLEAN');
  let state = await readState(tasks, cwd);
  let target = state.tasks[item.taskId];
  if (!target || target.state !== 'MERGE_QUEUED')
    throw new Error('TASK_NOT_MERGE_QUEUED');
  const credential = {
    taskId: item.taskId,
    holder: item.holder,
    leaseId: item.leaseId,
    fencingVersion: item.fencingVersion,
  };
  assertLeaseCredential(state, credential, now);
  assertPathLocks(task, tasks, state, now);
  ...
  const queueOperations = [
    'read-queue-item',
    'validated-lease-and-fence',
    'validated-path-locks',
    'fetched-cluster-head',
  ];
  if (rebaseApplied) {
    const previousLifecycle = await readLifecycleBinding(cwd, target);
    if (!previousLifecycle)
      throw new Error('PRE_REBASE_LIFECYCLE_BINDING_MISSING');
    ...
    staleEvidence.push(
      ...invalidation.records.map(
        (record) => `${record.kind}:${record.sha256}`,
      ),
    );
    if (invalidation.records.length !== 3)
      throw new Error('PRE_REBASE_EVIDENCE_INVALIDATION_INCOMPLETE');
    ...
    const authority = await persistLifecycleAuthority({
      ...
      ...(previousLifecycle.contractMode === 'LEGACY' &&
      previousLifecycle.conformanceManifestPath
        ? {
            compatibilityConformanceManifestPath:
              previousLifecycle.conformanceManifestPath,
          }
        : {}),
      ...(previousLifecycle.contractMode === 'LEGACY' &&
      previousLifecycle.conformanceManifestSha256
        ? {
            compatibilityConformanceManifestSha256:
              previousLifecycle.conformanceManifestSha256,
          }
        : {}),
    });
    ...
    await writeState(state, cwd);
    const launchReceiptId = await persistPostRebaseReceipt(
      task,
      target,
      worktree,
      cwd,
    ...
    if (verification.status !== 0)
      throw new Error(
        `POST_REBASE_TASK_VERIFICATION_FAILED:${verification.stderr || verification.stdout}`,
      );
    ...
    queueOperations.push(
      'ran-real-task-verifier',
      'requeued-fresh-verified-result',
    );
  }
  state = await readState(tasks, cwd);
  target = state.tasks[item.taskId];
  ...
  const commit = git(['rev-parse', 'HEAD'], worktree);
  const tree = git(['rev-parse', 'HEAD^{tree}'], worktree);
  if (commit !== target.commit || tree !== target.tree)
    throw new Error('POST_REBASE_RESULT_MISMATCH');
  if (git(['rev-parse', taskBranch(task.id)], cwd) !== commit)
    throw new Error('TASK_COMMIT_NOT_REACHABLE_FROM_REBASED_BRANCH');
  if (
    Number(git(['rev-list', '--count', `${clusterBranch}..${commit}`], cwd)) !==
    1
  )
  ...
  const integration = (options.integrationRunner ?? verifyPostMergeIntegration)(
    cwd,
  );
  ...
  if (integration.status === 'FAIL') {
    ...
    if (restoredTree !== preMergeTree)
      throw new Error('AUTOMATIC_REVERT_DID_NOT_RESTORE_TREE');
    const audit = {
      schemaVersion: '1.0.0',
      taskId: task.id,
      taskCommit: commit,
    ...
    await writeQueue(queue, cwd);
    queueOperations.push(
      'created-automatic-revert',
      'persisted-integration-failure-audit',
    );
    return {
      taskId: task.id,
