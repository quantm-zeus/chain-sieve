...
const loadFrameworkTask = async (
  spec: ValidatedSpecification,
  sourceRoot: string,
): Promise<{ task: TaskContract; sourceText: string; conformance: { content: string; sha256: string } }> => {
  const sourceText = await readFile(
    join(sourceRoot, 'tasks/framework-sources/T-FW-AUTOPILOT.task.json'),
    'utf8',
  );
  ...
  const conformance = buildConformanceManifest({
    ...
    taskAcceptanceCriteria: source.acceptanceCriteria as string[],
    taskAcceptanceFacets: source.taskAcceptanceFacets as Array<{ facetId: string }>,
  ...
  const task = TaskContractSchema.parse({
    ...
    interfaceHashes: { 'framework-task-source': sha256(sourceText) },
    conformanceManifestPath: `artifacts/conformance/${String(source.id)}/manifest.json`,
    conformanceManifestSha256: conformance.sha256,
  });
  return { task, sourceText, conformance };
};
...
const buildOutputs = async (spec: ValidatedSpecification, sourceRoot: string): Promise<OutputMap> => {
  const outputs: OutputMap = new Map(); const baselinePaths = fixedBaselinePaths(sourceRoot); const { tasks: productTasks, acceptancePartition } = makeTasks(spec, baselinePaths); const framework = await loadFrameworkTask(spec, sourceRoot); const tasks = [...productTasks, framework.task].sort((left, right) => left.id.localeCompare(right.id)); const clusters = [...makeClusters(spec, productTasks, acceptancePartition), ClusterContractSchema.parse({ schemaVersion: '1.0.0', id: 'C-FW-AUTOPILOT', group: 'FW', title: 'Autopilot framework implementation', sourceHashes: spec.hashes, dependencies: [], tasks: [framework.task.id], requirements: framework.task.requirements, acceptanceCriteria: [], integrationAcceptanceCriteria: [], invariants: framework.task.invariants, entryCriteria: ['No product task has an active lease.', 'The framework source compiles without drift.'], exitCriteria: ['The framework task is independently verified and atomically committed.', 'The framework cluster is merged to main through review.'], verificationCommands: [{ command: 'pnpm cluster:verify C-FW-AUTOPILOT', expected: 'exit 0' }, { command: 'pnpm harness:verify', expected: 'exit 0' }], rollback: 'Revert the framework task commit; retain the manual lifecycle commands.' })].sort((left, right) => left.id.localeCompare(right.id)); const api = extractApi(spec.prd); const persistence = extractPersistence(spec.prd);
  const requirementTaskMap = Object.fromEntries(spec.manifest.requirements.map((requirement) => [requirement.id, productTasks.filter((task) => task.requirements.includes(requirement.id)).map((task) => task.id)]));
  ...
  add(`tasks/${framework.task.dependencyGroup}/${framework.task.id}.contract.json`, framework.task);
  add(framework.task.conformanceManifestPath!, framework.conformance.content);
  const frameworkContext = new Map<string, string>([
    ['task-contract.json', json(framework.task)],
    ['framework-task-source.json', framework.sourceText],
    ['requirements.json', json(framework.task.requirements.map((id) => ({ id, authority: 'framework-task-source' })))],
    ['acceptance-criteria.json', json({ taskLocal: [], facets: [] })],
    ['normative-excerpts.json', json({ source: 'tasks/framework-sources/T-FW-AUTOPILOT.task.json' })],
    ['implementation-brief.json', json({ specificationStatus: 'READY', deliverables: framework.task.deliverables, constraints: framework.task.constraints })],
    ['interface-plan.json', json({ interfaces: ['pnpm autopilot'], provider: 'codex-cli' })],
    ['behavior-test-matrix.json', json({ taskId: framework.task.id, requiredBehaviorClasses: ['positive', 'negative', 'degraded', 'replay', 'recovery', 'rollback', 'observability'] })],
    ['referenced-path-status.json', json(framework.task.writeSet.map((path) => ({ path, status: 'VALID_REFERENCE' })))],
    ['dependency-outputs.json', json({ dependencies: [], interfaceHashes: framework.task.interfaceHashes })],
  ]);
  const frameworkManifest = { schemaVersion: '1.0.0', taskId: framework.task.id, sourceHashes: spec.hashes, files: [...frameworkContext].map(([path, content]) => ({ path, sha256: sha256(content), bytes: Buffer.byteLength(content) })), conformanceManifestPath: framework.task.conformanceManifestPath, conformanceManifestSha256: framework.task.conformanceManifestSha256, generatedAt: spec.manifest.document.generatedAt };
  for (const [path, content] of frameworkContext) add(`artifacts/context/${framework.task.id}/${path}`, content);
  add(`artifacts/context/${framework.task.id}/context-manifest.json`, frameworkManifest);
...
const generatedRoots = ['artifacts/spec', 'artifacts/conformance', 'tasks/G0', 'tasks/G1', 'tasks/G2', 'tasks/G3', 'tasks/G4', 'tasks/G5', 'tasks/G6', 'tasks/G7', 'tasks/FW', 'tasks/generated', 'clusters/G0', 'clusters/G1', 'clusters/G2', 'clusters/G3', 'clusters/G4', 'clusters/G5', 'clusters/G6', 'clusters/G7', 'clusters/FW', 'docs/schemas'];
const generatedContextRoots = (outputs: OutputMap): string[] => unique([...outputs.keys()].filter((path) => /^artifacts\/context\/T-(?:G[0-7]|FW)-/.test(path)).map((path) => path.split('/').slice(0, 3).join('/')));
