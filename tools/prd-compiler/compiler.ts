...
export const validateGeneratedGoalCommands = (goal: string, scripts: Record<string, string>): string[] => {
  ...
  for (const command of commands) { if (!scripts[command.script]) throw new Error(`UNKNOWN_GENERATED_COMMAND:${command.script}`); const required = requiredGoalArguments[command.script]; if (required && !required.test(command.args)) throw new Error(`INVALID_GENERATED_COMMAND_ARGUMENTS:${command.script}:${command.args}`); }
  const lifecycle = ['task:acquire', 'task:begin', 'task:self-review', 'task:verify', 'merge-queue:add', 'merge-queue:process', 'cluster:verify', 'cluster:report']; const positions = lifecycle.map((script) => commands.findIndex((command) => command.script === script)); if (positions.some((position) => position < 0) || positions.some((position, index) => index > 0 && position <= positions[index - 1]!)) throw new Error('INVALID_GENERATED_GOAL_STATE_TRANSITIONS');
...
const buildOutputs = async (spec: ValidatedSpecification): Promise<OutputMap> => {
  ...
  if (Object.values(acceptanceTaskMap).some((ids) => ids.length !== 1)) throw new Error('ACCEPTANCE_OWNERSHIP_NOT_UNIQUE');
  ...
  for (const [path, content] of outputs) if (path.endsWith('.zcode-goal.md') || path.endsWith('.zcode-fix-goal.md') || path.endsWith('.codex-review.md')) validateGeneratedGoalCommands(content, packageManifest.scripts);
...
const generatedRoots = ['artifacts/spec', 'tasks/G0', 'tasks/G1', 'tasks/G2', 'tasks/G3', 'tasks/G4', 'tasks/G5', 'tasks/G6', 'tasks/G7', 'tasks/generated', 'clusters/G0', 'clusters/G1', 'clusters/G2', 'clusters/G3', 'clusters/G4', 'clusters/G5', 'clusters/G6', 'clusters/G7', 'docs/schemas'];
