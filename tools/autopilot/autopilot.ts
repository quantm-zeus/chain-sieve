...
const ensureLease = async (
  ...
  const head = requireSuccess(
    runner.run('git', ['rev-parse', 'HEAD'], { cwd: workspace }),
    'AUTOPILOT_HEAD_FAILED',
  );
  const tree = requireSuccess(
    runner.run('git', ['rev-parse', 'HEAD^{tree}'], { cwd: workspace }),
    'AUTOPILOT_TREE_FAILED',
  );
    ...
    for (
      ...
      if (reviewing) {
        const reviewPath = join(
          ...
          'cluster-reviews',
          `${reviewing.contract.id}.review-instructions.md`,
        );
        const result = provider.executePayload(
          reviewing.branch.worktree,
          `Read and obey ${reviewPath}. Perform the independent review autonomously, create the exact review artifact, commit only that artifact, and stop.`,
        );
