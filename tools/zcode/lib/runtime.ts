...
export const zcodeRuntimeRoot = (root: string, runner: CommandRunner): string =>
  join(gitCommonDirectory(root, runner), 'ciag-runtime', 'zcode');
...
const writeImmutable = async (path: string, content: string): Promise<void> => {
  ...
  try {
    ...
    if ((await readFile(path, 'utf8')) !== content)
      throw new ZCodeError('IMMUTABLE_ZCODE_EVIDENCE_COLLISION', path);
  }
};
...
Do not renew the lease inside this ZCode session because renewal changes the proof bindings. Pause before the renewal window and ask the owner to run the same root \`pnpm zcode\` command; the orchestrator will renew through the authoritative lifecycle, regenerate the receipt and goal, and resume this task.
...
export const persistGoalAndPayload = async (
  ...
  const runtime = zcodeRuntimeRoot(root, runner);
  ...
  const payload = generatePayload(finalBinding);
  ...
  const receiptPath = join(
    ...
    'launch-receipts',
    binding.task.contract.id,
    `${sha256(`${binding.leaseId}:${binding.fencingVersion}`)}.json`,
  );
  const receipt = {
    schemaVersion: '1.0.0',
...
export const validateLaunchReceipt = async (
  ...
  const expectedGoalRoot = join(runtime, 'goals', binding.taskId);
