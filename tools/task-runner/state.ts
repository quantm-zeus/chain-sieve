...
export const readState = async (tasks: LeaseContract[], cwd = process.cwd()): Promise<LifecycleDocument> => {
  try { const value = JSON.parse(await readFile(statePath(cwd), 'utf8')) as LifecycleDocument; if (value.schemaVersion !== '1.0.0' || typeof value.tasks !== 'object') throw new Error('TASK_STATE_INVALID'); for (const task of tasks) if (!value.tasks[task.id]) value.tasks[task.id] = { taskId: task.id, state: task.dependencies.length === 0 ? 'READY' : 'PLANNED', leaseVersion: 0 }; return value; }
...
export const acquire = (state: LifecycleDocument, tasks: LeaseContract[], taskId: string, holder: string, now: Date, ttlMs = 900_000, baseCommit?: string, branch?: string): ReturnType<typeof TaskLeaseSchema.parse> => {
...
export const refreshReady = (state: LifecycleDocument, tasks: LeaseContract[]): void => { for (const task of tasks) { const target = state.tasks[task.id]; if (target?.state === 'PLANNED' && task.dependencies.every((dependency) => state.tasks[dependency]?.state === 'MERGED')) target.state = 'READY'; } };
