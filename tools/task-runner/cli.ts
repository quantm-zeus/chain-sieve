...
try {
  ...
  if (command === 'list') console.log(JSON.stringify(Object.values(state.tasks), null, 2));
    ...
    else if (command === 'begin') { const expected = repairTasks.find((task) => task.id === taskId)?.approvedBranch ?? `task/${taskId.toLowerCase()}`; const branch = git(['branch', '--show-current']); if (branch !== expected || target.branch !== expected) throw new Error(`TASK_BRANCH_MISMATCH:${branch}`); if (git(['status', '--porcelain']) !== '') throw new Error('DIRTY_WORKTREE'); if (git(['rev-parse', 'HEAD']) !== target.baseCommit) throw new Error('TASK_BASE_COMMIT_MISMATCH'); if (!repairTasks.some((task) => task.id === taskId)) await verifyTaskContract(taskId); transition(target, ['LEASED'], 'IMPLEMENTING'); }
