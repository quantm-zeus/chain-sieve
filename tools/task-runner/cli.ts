...
try {
  ...
  else if (command === 'acquire') { const expected = `task/${taskId.toLowerCase()}`; const branch = git(['branch', '--show-current']); if (branch !== expected) throw new Error(`TASK_BRANCH_REQUIRED:${expected}`); const lease = acquire(state, tasks, taskId, holder, new Date(), 900_000, git(['rev-parse', 'HEAD']), branch); await writeState(state); console.log(JSON.stringify(lease, null, 2)); }
