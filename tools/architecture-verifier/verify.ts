...
const sourceFiles = async (): Promise<string[]> => (await Promise.all(architectureRoots.map((root) => walk(join(process.cwd(), root))))).flat();
const executableFiles = async (): Promise<string[]> => (await Promise.all(executableRoots.map((root) => walkExecutable(join(process.cwd(), root))))).flat();
...
export const scanPlaceholders = async (): Promise<{ files: number }> => {
  const violations: string[] = []; const files = (await executableFiles()).filter((file) => relative(process.cwd(), file) !== 'tools/architecture-verifier/verify.ts');
  for (const file of files) { const path = relative(process.cwd(), file); const text = await readFile(file, 'utf8'); violations.push(...placeholderViolations(path, text)); }
...
export const scanProhibitedCapabilities = async (): Promise<{ files: number }> => {
  const violations: string[] = []; const files = (await executableFiles()).filter((file) => { const path = relative(process.cwd(), file); return path !== 'tools/architecture-verifier/verify.ts' && !path.startsWith('packages/security/') && !path.startsWith('packages/provider-contracts/') && !path.startsWith('tests/'); });
  for (const file of files) { const path = relative(process.cwd(), file); const text = await readFile(file, 'utf8'); violations.push(...prohibitedCapabilityViolations(path, text)); }
