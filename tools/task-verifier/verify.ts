...
export const verifyCoverage = async (): Promise<{ requirements: number; acceptanceCriteria: number; tasks: number }> => { const spec = await loadAndValidateSpecification(); const tasks = await loadTasks(); const requirementCounts = new Map<string, number>(); const acceptanceCounts = new Map<string, number>(); for (const task of tasks) { for (const id of task.requirements) requirementCounts.set(id, (requirementCounts.get(id) ?? 0) + 1); for (const id of task.acceptanceCriteria) acceptanceCounts.set(id, (acceptanceCounts.get(id) ?? 0) + 1); } const badRequirements = spec.manifest.requirements.filter((item) => requirementCounts.get(item.id) !== 1); const badAcceptance = spec.manifest.acceptanceCriteria.filter((item) => acceptanceCounts.get(item.id) !== 1); if (badRequirements.length || badAcceptance.length) throw new Error(`INCOMPLETE_OR_DUPLICATE_COVERAGE:R=${badRequirements.length}:A=${badAcceptance.length}`); return { requirements: requirementCounts.size, acceptanceCriteria: acceptanceCounts.size, tasks: tasks.length }; };
...
export const verifyTaskContract = async (taskId: string): Promise<{ taskId: string; evidenceHash: string }> => {
  ...
  const expectedTests = [...new Set(acceptance.flatMap((item) => [item.positiveTestRef, item.negativeOrFailureTestRef]))]; if (JSON.stringify(task.requiredTests) !== JSON.stringify(expectedTests)) throw new Error(`TASK_TEST_MAPPING_DRIFT:${task.id}`);
...
export const verifyTask = async (taskId: string, holder: string, leaseVersion: number): Promise<{ taskId: string; commit: string; evidenceHash: string }> => {
  ...
  try {
    ...
    for (const path of changed) { if (task.forbiddenPaths.some((pattern) => covers(pattern, path))) throw new Error(`FORBIDDEN_PATH_CHANGE:${path}`); if (!task.allowedPaths.some((pattern) => covers(pattern, path))) throw new Error(`PATH_OUTSIDE_TASK_SCOPE:${path}`); }
    const changedLines = git(['diff', '--numstat', `${base}..${commit}`]).split('\n').filter(Boolean).reduce((sum, line) => sum + line.split('\t').slice(0, 2).reduce((count, value) => count + (Number(value) || 0), 0), 0); if (changedLines > task.complexityBudget.maxChangedLines) throw new Error('TASK_LINE_BUDGET_EXCEEDED');
    for (const path of task.requiredTests) { if (!path.startsWith('tests/') || path.includes('..')) throw new Error(`UNSAFE_TEST_PATH:${path}`); await access(path); const text = await readFile(path, 'utf8'); const violations = placeholderViolations(path, text); if (violations.length > 0) throw new Error(`INVALID_TASK_TEST:${violations.join(',')}`); }
