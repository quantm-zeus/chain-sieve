...
export const loadTasks = async (): Promise<TaskContract[]> => { await waitForCompilerIdle(); const tasks: TaskContract[] = []; for (const group of [...Array.from({ length: 8 }, (_, index) => `G${index}`), 'FW']) { const root = join(process.cwd(), `tasks/${group}`); for (const file of await readdir(root).catch(() => [])) if (file.endsWith('.contract.json')) tasks.push(TaskContractSchema.parse(JSON.parse(await readFile(join(root, file), 'utf8')))); } return tasks.sort((left, right) => left.id.localeCompare(right.id)); };
export const loadClusters = async (): Promise<ClusterContract[]> => { await waitForCompilerIdle(); const clusters: ClusterContract[] = []; for (const group of [...Array.from({ length: 8 }, (_, index) => `G${index}`), 'FW']) { const root = join(process.cwd(), `clusters/${group}`); for (const file of await readdir(root).catch(() => [])) if (file.endsWith('.contract.json')) clusters.push(ClusterContractSchema.parse(JSON.parse(await readFile(join(root, file), 'utf8')))); } return clusters.sort((left, right) => left.id.localeCompare(right.id)); };

export const verifyCoverage = async (): Promise<{ requirements: number; acceptanceCriteria: number; tasks: number }> => { const spec = await loadAndValidateSpecification(); const tasks = await loadTasks(); const productTasks = tasks.filter((task) => task.dependencyGroup !== 'FW'); const requirementCounts = new Map<string, number>(); for (const task of productTasks) for (const id of task.requirements) requirementCounts.set(id, (requirementCounts.get(id) ?? 0) + 1); const levels = JSON.parse(await readFile('tasks/generated/acceptance-level-map.json', 'utf8')) as Record<string, { level: 'TASK' | 'CLUSTER' | 'PROJECT'; owner: string }>; const badRequirements = spec.manifest.requirements.filter((item) => requirementCounts.get(item.id) !== 1); const badAcceptance = spec.manifest.acceptanceCriteria.filter((item) => !levels[item.id]); const unknownAcceptance = Object.keys(levels).filter((id) => !spec.manifest.acceptanceCriteria.some((item) => item.id === id)); const taskIds = new Set(productTasks.map((task) => task.id)); const clusters = await loadClusters(); const clusterIds = new Set(clusters.map((cluster) => cluster.id)); const invalidOwners = Object.entries(levels).filter(([, value]) => value.level === 'TASK' ? !taskIds.has(value.owner) : value.level === 'CLUSTER' ? !clusterIds.has(value.owner) : value.owner !== 'PROJECT'); if (badRequirements.length || badAcceptance.length || unknownAcceptance.length || invalidOwners.length) throw new Error(`INCOMPLETE_OR_DUPLICATE_COVERAGE:R=${badRequirements.length}:A=${badAcceptance.length + unknownAcceptance.length + invalidOwners.length}`); return { requirements: requirementCounts.size, acceptanceCriteria: Object.keys(levels).length, tasks: tasks.length }; };
...
export const verifyTaskContract = async (taskId: string): Promise<{ taskId: string; evidenceHash: string }> => {
  ...
  if (task.dependencyGroup === 'FW') {
    const sourceText = await readFile(`tasks/framework-sources/${task.id}.task.json`, 'utf8');
    if (task.interfaceHashes['framework-task-source'] !== sha256(sourceText)) throw new Error(`FRAMEWORK_TASK_SOURCE_DRIFT:${task.id}`);
    if (task.requiredTests.length === 0) throw new Error(`FRAMEWORK_TASK_TESTS_MISSING:${task.id}`);
  } else {
...
export const verifyTask = async (
  ...
  if (task.dependencyGroup !== 'FW') assertTrustedTargetPaths(changed);
