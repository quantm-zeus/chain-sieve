...
export class ShadowNotificationAdapter implements NotificationAdapter {
  ...
  async enqueue(input: { outboxId: string; template: string }): Promise<DegradedResult<{ deliveryId: string }>> { this.messages.push(`${input.outboxId}:${input.template}`); return { status: 'AVAILABLE', capabilityMode: 'SYNTHETIC_SHADOW', value: { deliveryId: `shadow-${input.outboxId}` } }; }
}
