# C-G4-IMPLEMENTATION zcode-fix-goal

Fix only the findings in the latest cluster review. The immutable contract is `C-G4-IMPLEMENTATION.contract.json`. Validate source hashes before work. Work only on `cluster/g4`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
