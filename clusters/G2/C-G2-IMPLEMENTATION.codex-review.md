...
Independently review the completed cluster and produce PASS or CHANGES_REQUIRED. The immutable contract is `C-G2-IMPLEMENTATION.contract.json`. Validate source hashes before work. Work only on `cluster/g2`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
...
After all tasks are atomically merged, the root control plane runs `pnpm cluster:verify C-G2-IMPLEMENTATION`, freezes the cluster result, and only then requests a separate independent review before `pnpm cluster:report C-G2-IMPLEMENTATION`.
