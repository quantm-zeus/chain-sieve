# C-G6-IMPLEMENTATION agent-goal

Implement each READY task one at a time. The immutable contract is `C-G6-IMPLEMENTATION.contract.json`. Validate source hashes before work. Work only on `cluster/g6`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
...
After all tasks are atomically merged, run `pnpm cluster:verify C-G6-IMPLEMENTATION`. Cluster completion still requires a separate independent Codex review and `pnpm cluster:report C-G6-IMPLEMENTATION`; the implementation agent cannot author its own review or merge to main.
