# C-G5-IMPLEMENTATION agent-goal

Implement each READY task one at a time. The immutable contract is `C-G5-IMPLEMENTATION.contract.json`. Validate source hashes before work. Work only on `cluster/g5`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
