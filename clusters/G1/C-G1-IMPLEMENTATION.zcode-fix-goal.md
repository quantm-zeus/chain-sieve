# C-G1-IMPLEMENTATION zcode-fix-goal

Fix only the findings in the latest cluster review. The immutable contract is `C-G1-IMPLEMENTATION.contract.json`. Validate source hashes before work. Work only on `cluster/g1`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
...
After all tasks are atomically merged, run `pnpm cluster:verify C-G1-IMPLEMENTATION`. Cluster completion still requires a separate Codex review and `pnpm cluster:report C-G1-IMPLEMENTATION`; this goal cannot author its own review or merge to main.
