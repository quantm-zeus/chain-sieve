# C-FW-AUTOPILOT agent-goal

Implement each READY task one at a time. The immutable contract is `C-FW-AUTOPILOT.contract.json`. Validate source hashes before work. Work only on `cluster/fw`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
