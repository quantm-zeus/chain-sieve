# C-FW-AUTOPILOT codex-review

Independently review the completed cluster and produce PASS or CHANGES_REQUIRED. The immutable contract is `C-FW-AUTOPILOT.contract.json`. Validate source hashes before work. Work only on `cluster/fw`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
...
After all tasks are atomically merged, the root control plane runs `pnpm cluster:verify C-FW-AUTOPILOT`, freezes the cluster result, and only then requests a separate independent review before `pnpm cluster:report C-FW-AUTOPILOT`.
