# C-G0-IMPLEMENTATION codex-review

Independently review the completed cluster and produce PASS or CHANGES_REQUIRED. The immutable contract is `C-G0-IMPLEMENTATION.contract.json`. Validate source hashes before work. Work only on `cluster/g0`; create it from the current verified baseline if it does not exist. Never switch to or merge into `main`.
...
After all tasks are atomically merged, run `pnpm cluster:verify C-G0-IMPLEMENTATION`. Cluster completion still requires a separate independent Codex review and `pnpm cluster:report C-G0-IMPLEMENTATION`; the implementation agent cannot author its own review or merge to main.
