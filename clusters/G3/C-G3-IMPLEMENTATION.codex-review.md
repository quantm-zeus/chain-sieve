...
After all tasks are atomically merged, the root control plane runs `pnpm cluster:verify C-G3-IMPLEMENTATION`, freezes the cluster result, and only then requests a separate independent review before `pnpm cluster:report C-G3-IMPLEMENTATION`.
