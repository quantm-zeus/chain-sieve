...
const setup = async (
  ...
  const result = ClusterResultSchema.parse({
    ...
    status: 'PASS',
    clusterContractSha256: 'a'.repeat(64),
    headCommitSha: productCommit,
    headTreeSha: productTree,
    taskAttestations: [
      {
        taskId: 'T-G0-CORE',
        taskCommitSha: productCommit,
        taskTreeSha: productTree,
    ...
    commandEvidence: [
      {
        ...
        artifactPath: 'evidence.log',
        artifactSha256: 'c'.repeat(64),
      },
    ],
    verifierVersion: 'cluster-2.0.0',
    verificationPolicyVersion: 'cluster-policy',
    verificationTimestamp: '2026-07-31T00:00:00.000Z',
  });
  const resultText = `${JSON.stringify(result, null, 2)}\n`;
  await put(
    runtimeRoot(root),
    `cluster-results/${cluster.id}.result.json`,
    resultText,
  );
  const reviewerIdentity = 'independent-session-42';
  const receiptCore = {
    schemaVersion: '2.0.0' as const,
    sessionId: 'review-session-42',
    reviewerIdentity,
    provider: 'antigravity' as const,
    clusterId: cluster.id,
    productCommit,
    productTree,
    implementationHolders: [] as string[],
  ...
  await put(
    runtimeRoot(root),
    `agent/review-sessions/${cluster.id}.review-session-42.json`,
  ...
  const review = {
    ...
    clusterId: cluster.id,
    reviewerIdentity,
    reviewedProductCommit: productCommit,
    reviewedProductTree: productTree,
    clusterContractSha256: result.clusterContractSha256,
    clusterResultPath: `cluster-results/${cluster.id}.result.json`,
    clusterResultSha256: sha256(resultText),
    taskAttestationHashes: [
      { taskId: 'T-G0-CORE', resultSha256: 'b'.repeat(64) },
    ],
    ...
    reviewPasses: [
      {
        ...
        status: 'PASS',
        evidence: ['commit/tree/result/task attestations checked'],
      },
    ],
    findings: [],
    verdict: 'PASS',
  ...
  if (extraReviewFile)
    await put(root, 'product-tamper.txt', 'not metadata only\n');
  git(root, ['add', '.']);
...
describe('cluster independent-review binding', () => {
  ...
  it.each([
    [
      'reviewedProductCommit',
      '1'.repeat(40),
      'CLUSTER_REVIEW_WRONG_PRODUCT_COMMIT',
    ],
    [
      'reviewedProductTree',
      '2'.repeat(40),
      'CLUSTER_REVIEW_WRONG_PRODUCT_TREE',
    ],
    [
      'clusterResultSha256',
      '3'.repeat(64),
      'CLUSTER_REVIEW_WRONG_RESULT_HASH',
    ],
    [
      'reviewedAt',
      '2026-07-30T00:00:00.000Z',
      'CLUSTER_REVIEW_PREDATES_RESULT',
    ],
  ...
  it('rejects unresolved P1 findings and non-review changes', async () => {
    const finding = await setup({
      findings: [
        { severity: 'P1', text: 'release blocker', resolved: false },
      ],
    });
    await expect(
      validateClusterReview(
    ...
    ).rejects.toThrow('CLUSTER_INDEPENDENT_REVIEW_REQUIRED');
    ...
    ).rejects.toThrow('CLUSTER_REVIEW_COMMIT_NOT_ARTIFACT_ONLY');
  });
