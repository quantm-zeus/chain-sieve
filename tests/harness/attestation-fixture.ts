...
import {
  ...
  sha256,
} from '../../tools/prd-compiler/compiler.js';
import {
...
const git = (cwd: string, args: string[]): string => {
  ...
  if (result.status !== 0)
    throw new Error(
      `FIXTURE_GIT_FAILED:${args.join(':')}:${result.stderr.trim()}`,
    );
...
export const createAttestationFixture =
  async (): Promise<AttestationFixture> => {
    ...
    await put(
      root,
      `tasks/${task.dependencyGroup}/${task.id}.contract.json`,
      contractText,
    );
    if (task.conformanceManifestPath) {
      await put(
        root,
        task.conformanceManifestPath,
        `${git(process.cwd(), ['show', `HEAD:${task.conformanceManifestPath}`])}\n`,
      );
    }
    await put(
      root,
    ...
    for (const path of task.requiredTests)
      await put(root, path, `// immutable acceptance fixture for ${path}\n`);
      ...
      [
        {
          command,
          exitCode: 0,
          output: 'all mapped tests passed\n',
    ...
    const result = TaskResultSchema.parse({
      ...
      bindings: {
        ...
        headTreeSha: tree,
        changedFiles,
        requirementToCode: deriveRequirementMapping(
          task,
          changedFiles,
          head,
          root,
    ...
    return {
      ...
      removeFirstArtifact: async () =>
        unlink(join(runtimeRoot(root), commandEvidence[0]!.artifactPath)),
