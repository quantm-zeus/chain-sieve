...
describe('authoritative bootstrap migration', () => {
  ...
  it('keeps authoritative SQL table names mirrored by Drizzle without speculative production schema', async () => { const sql = await readFile('infra/migrations/0001_bootstrap_foundation.sql', 'utf8'); const sqlTables = [...sql.matchAll(/CREATE TABLE IF NOT EXISTS\s+([a-z_]+)/g)].map((match) => match[1]).sort(); const drizzleTables = Object.values(schema).map((table) => getTableName(table)).sort(); expect(drizzleTables).toEqual(sqlTables); expect(sqlTables).toHaveLength(10); });
});
